...
epochs = 200000  # Try more!
lr = 1e-4
T = 500

rgb = [50, 60, 100]
# model_load_path = "spectraldiff_diffusion-master/save_model/IP_diffusion_uvit"
# model_load_path = "save_model/PU_diffusion_uvit"
model_load_path = "save_model/Houston_diffusion_uvit"

# model_name = "uvit3d_99700.pkl"  # loss=0.0364
model_name = "uvit3d_8000.pkl"  # loss=0.0347

# save_feature_path_prefix = "save_feature/IP_uvit"
# save_feature_path_prefix = "save_feature/PU_uvit_23000"
save_feature_path_prefix = "save_feature/Houston_uvit_8000"
...
def inference_mini_batch(model, xt, t):
    ...
    for i in range(step):
        ...
        if temp_xt.shape[0] <= 0:
            break
        noise_pred = model(temp_xt, t, feature=True)            # gaigaigiagigiaigiagaigaigaigaigagaigiagiagigiagiaiggiagigaigaigai
...
def inference_by_t(dataloader, diffusion, model, X, ti):
    ...
    for index, feature_matrix in enumerate(feature_t_list):
        ...
        # feature_matrix = feature_matrix[:, 2:, :]           # gaigaigaigaigai
        # print("feature matrix shape:", feature_matrix.shape)
        fb, fc, fs = feature_matrix.shape
        fh = fw = int(fc ** .5)
        # spe = fs//2

        temp = feature_matrix.reshape((fb, fh, fw, fs))
        # print(temp.shape)
    ...
    _, recon_from_xt = diffusion.reconstruct(model, xt=show_xt, tempT=t,
                                             num=5)  # recon_from_xt[0] shape (batch, channel, spe, h, w)
    recon_x0 = recon_from_xt[-1]
...
def extract():
    # dataloader = HSIDataLoader({"data": {"data_sign": "Indian", "padding": False, "batch_size": batch_size,
    #                                      "patch_size": patch_size, "select_spectral": select_spectral}})
    ...
    # dataloader = HSIDataLoader({"data":{"data_sign":"HongHu", "padding":False, "batch_size":batch_size, "patch_size":patch_size, "select_spectral":select_spectral}})
    ...
    # model = UViT(in_chans=channel, num_spe=spe, embed_dim=spe)
    model = UViT(in_chans=channel, num_spe=spe)
