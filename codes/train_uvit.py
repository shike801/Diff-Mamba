...
epochs = 30000  # more than 30000
lr = 1e-4
T = 500

rgb = [30, 50, 90]
# path_prefix = "spectraldiff_diffusion-master/save_model/%s_diffusion_uvit" % sign
# path_prefix = "spectraldiff_diffusion-master/save_model/%s_diffusion_uvit2" % sign
path_prefix = "save_model/%s_diffusion_uvit" % sign
...
def train():
    ...
    #     {"data": {"data_sign": "Indian", "padding": False, "batch_size": batch_size, "patch_size": patch_size,
    #               "select_spectral": select_spectral}})
    # dataloader = HSIDataLoader(
    #     {"data": {"data_sign": "Pavia", "padding": False, "batch_size": batch_size, "patch_size": patch_size,
    #               "select_spectral": select_spectral}})
    # dataloader = HSIDataLoader(
    #     {"data": {"data_sign": "HongHu", "padding": False, "batch_size": batch_size, "patch_size": patch_size,
    #               "select_spectral": select_spectral}})
    dataloader = HSIDataLoader(
        {"data": {"data_sign": "Houston", "padding": False, "batch_size": batch_size, "patch_size": patch_size,
                  "select_spectral": select_spectral}})
    ...
    model = UViT(in_chans=channel, num_spe=spe,patch_size=patch_size)
    model.to(device)
    ...
    for epoch in range(epochs):
        ...
        if epoch % 100 == 0:
            ...
            path = "%s/uvit3d_%s.pkl" % (path_prefix, epoch)
            save_model(model, path)
            # save_model(model, optimizer, epoch, path)
